...
import '../../providers/eod_providers.dart';

/// Backend permission required to see the Team EOD tab. Kept local because the
/// mobile `Permissions` class hasn't yet been extended with EOD constants —
/// same workaround used by the standup screen for STANDUP_TEAM_VIEW.
const String _kEodTeamViewPerm = 'EOD_TEAM_VIEW';
...
class _EodScreenState extends ConsumerState<EodScreen>
  ...
  void _syncTabController(bool canSeeTeam) {
    if (_tabController != null && canSeeTeam == _canSeeTeam) return;
    _canSeeTeam = canSeeTeam;
    _tabController?.dispose();
    _tabController = TabController(length: canSeeTeam ? 3 : 2, vsync: this);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final canTeam = user?.hasPermission(_kEodTeamViewPerm) == true;
    _syncTabController(canTeam);
    final tabs = <Tab>[
      const Tab(text: 'Submit'),
      const Tab(text: 'My History'),
      if (canTeam) const Tab(text: 'Team EOD'),
    ];
...
// ── Team EOD Tab ─────────────────────────────────────────────────────────────
//
// Mirrors the Standup "Team Standups" tab: filter card (date presets + custom
// range + project + user + clear) on top, scrollable list of EOD cards in the
// middle, server-driven pagination footer at the bottom. The User dropdown is
// populated from the full team-peer roster (every co-member / lead / report),
// not just users who have an entry on the visible page.

enum _EodDatePreset { today, yesterday, week, all, custom }

class _TeamEodTab extends ConsumerStatefulWidget {
  const _TeamEodTab();

  @override
  ConsumerState<_TeamEodTab> createState() => _TeamEodTabState();
}

class _TeamEodTabState extends ConsumerState<_TeamEodTab> {
  // Default range = today (matches Standup behaviour).
  ...
  void _applyPreset(_EodDatePreset preset) {
    final now = DateTime.now();
    setState(() {
      _page = 1;
      switch (preset) {
        case _EodDatePreset.today:
          final d = DateFormat('yyyy-MM-dd').format(now);
          _dateFrom = d; _dateTo = d;
          break;
        case _EodDatePreset.yesterday:
          final d = DateFormat('yyyy-MM-dd').format(
              now.subtract(const Duration(days: 1)));
          _dateFrom = d; _dateTo = d;
          break;
        case _EodDatePreset.week:
          // Week starts Monday — match the Standup tab.
          ...
          break;
        case _EodDatePreset.all:
          _dateFrom = ''; _dateTo = '';
          break;
        case _EodDatePreset.custom:
          break;
      }
    });
  }

  _EodDatePreset get _activePreset {
    final now = DateTime.now();
    ...
    if (_dateFrom.isEmpty && _dateTo.isEmpty)         return _EodDatePreset.all;
    if (_dateFrom == t  && _dateTo == t)              return _EodDatePreset.today;
    if (_dateFrom == y  && _dateTo == y)              return _EodDatePreset.yesterday;
    if (_dateFrom == ws && _dateTo == we)             return _EodDatePreset.week;
    return _EodDatePreset.custom;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final teamAsync = ref.watch(teamEodsProvider(params));

    return RefreshIndicator(
      onRefresh: () async => ref.invalidate(teamEodsProvider(params)),
      child: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          // Info banner
          Container(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(
                child: Text(
                  "Showing EODs from members of teams you're in or lead.",
                  style: TextStyle(
          ...
          teamAsync.when(
            ...
            error: (e, _) => Padding(
              ...
              child: Column(children: [
                ...
                const SizedBox(height: 12),
                ElevatedButton(
                  onPressed: () => ref.invalidate(teamEodsProvider(params)),
            ...
            data: (result) {
              if (result.entries.isEmpty) {
                return Padding(
                  ...
                  child: Column(children: [
                    ...
                    Text('No team EODs in this range',
                        style: TextStyle(
              ...
              return Column(
                children: [
                  ...result.entries.map((e) => _TeamEodCard(entry: e)),
                  const SizedBox(height: 10),
                  _EodPaginationFooter(
                    page:       _page,
                    pageSize:   _pageSize,
                    total:      result.total,
  ...
  // The User filter is sourced from the full team-peer roster (every user the
  // caller shares a team with) — keeps the dropdown complete even when peers
  // haven't posted on the visible page.
...
// ── Team EOD filter card ─────────────────────────────────────────────────────

class _EodTeamFilterCard extends StatelessWidget {
  const _EodTeamFilterCard({
  ...
  final _EodDatePreset activePreset;
  final ValueChanged<_EodDatePreset> onPreset;
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              _EodPresetChip(
                label: 'Today',
                active: activePreset == _EodDatePreset.today,
                onTap: () => onPreset(_EodDatePreset.today),
              ),
              _EodPresetChip(
                label: 'Yesterday',
                active: activePreset == _EodDatePreset.yesterday,
                onTap: () => onPreset(_EodDatePreset.yesterday),
              ),
              _EodPresetChip(
                label: 'This Week',
                active: activePreset == _EodDatePreset.week,
                onTap: () => onPreset(_EodDatePreset.week),
              ),
              _EodPresetChip(
                label: 'All Time',
                active: activePreset == _EodDatePreset.all,
                onTap: () => onPreset(_EodDatePreset.all),
              ),
              if (activePreset == _EodDatePreset.custom)
                Container(
          ...
          // User dropdown — populated from the full team-peer roster.
          Text('User',
              style: TextStyle(
                  fontSize: 11, fontWeight: FontWeight.w700,
                  color: ds.textSecondary, letterSpacing: 0.3)),
          ...
          DropdownButtonFormField<String>(
            // If the saved selection doesn't appear in the roster (e.g. peer
            // removed from team while still selected), fall back to "All
            // users" rather than asserting on a missing dropdown value.
...
class _EodPresetChip extends StatelessWidget {
  const _EodPresetChip({
    required this.label,
...
class _EodDatePickerField extends StatelessWidget {
  const _EodDatePickerField({
    required this.label,
    required this.value,
    required this.onTap,
  });
...
class _TeamEodCard extends StatelessWidget {
  const _TeamEodCard({required this.entry});
  final Map<String, dynamic> entry;
  ...
  Widget build(BuildContext context) {
    ...
    final plannedTomorrow = entry['plannedTomorrow']?.toString();
    final blockers        = entry['blockers']?.toString();
    final mood            = (entry['mood'] ?? 'GREEN').toString();
    final progress = int.tryParse(
            entry['progressPercentage']?.toString().split('.').first ?? '0') ??
        0;
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Header — avatar + name + project + date + mood + progress chip
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              UserAvatar(
...
class _EodPaginationFooter extends StatelessWidget {
  const _EodPaginationFooter({
    required this.page,
...
class _EodPageBtn extends StatelessWidget {
  const _EodPageBtn({
    required this.label,
