...
class CronController {
  ...
  static async approvalReminder(req, res) {
    ...
    for (const a of pending) {
      ...
      if (rmRows[0]) {
        await notif.send({ toEmail: rmRows[0].email, subject: 'Time approval reminder', htmlBody: `<p>Hi ${rmRows[0].name}, ${requesterRows[0]?.name || 'a team member'} has a time entry pending your approval for over 2 days.</p>` });
