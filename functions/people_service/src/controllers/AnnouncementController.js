...
class AnnouncementController {
  ...
  async create(req, res) {
    try {
      const { title, content, type, target_roles, target_user_ids, is_pinned, priority, expires_at } = req.body;
      if (!title || !content) return ResponseHelper.validationError(res, 'title and content required');

      const row = await this.db.insert(TABLES.ANNOUNCEMENTS, {
        tenant_id: req.tenantId, title, content,
        type: type || 'GLOBAL',
        ...
        ...(expires_at ? { expires_at } : {}), view_count: 0,
  ...
  async update(req, res) {
    try {
      ...
      const allowed = ['title', 'content', 'is_pinned', 'announcement_priority', 'expires_at', 'target_roles', 'target_user_ids'];
      const updates = {};
      allowed.forEach(f => { if (req.body[f] !== undefined) updates[f] = Array.isArray(req.body[f]) ? JSON.stringify(req.body[f]) : req.body[f]; });
      if (req.body.priority !== undefined) updates.announcement_priority = req.body.priority;
      const updated = await this.db.update(TABLES.ANNOUNCEMENTS, { ROWID: req.params.id, ...updates });
