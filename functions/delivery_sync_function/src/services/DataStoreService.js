...
class DataStoreService {
   ...
   * @param {string} tableName
   * @param {string} tenantId
   * @param {string} whereExtra  – additional WHERE clauses (without tenant_id)
   * @param {object} options     – { page=1, pageSize=20, orderBy='CREATEDTIME DESC' }
   */
  async findPaginated(tableName, tenantId, whereExtra, options = {}) {
    ...
    // Count total (for totalPages calculation)
    let total = 0;
    try {
      const countRows = await this.query(
        `SELECT COUNT(ROWID) FROM ${tableName} WHERE ${fullWhere}`
      );
      if (countRows.length > 0) {
        total = parseInt(Object.values(countRows[0])[0], 10) || 0;
      }
  ...
  /**
   * Count rows with a raw WHERE clause + tenant isolation.
   */
  ...
  /**
   * Format a Date object as 'YYYY-MM-DD HH:MM:SS' for Catalyst DateTime columns.
   */
  static fmtDT(date) {
