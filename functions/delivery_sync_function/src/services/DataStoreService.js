...
class DataStoreService {
  ...
  async query(zcqlQuery) {
    try {
      ...
      console.log('[ZCQL RAW]', zcqlQuery.substring(0, 80), '→', JSON.stringify(result && result[0]));
  ...
  async insert(tableName, data) {
    try {
      ...
      console.error('[DataStoreService] Insert error:', tableName, err.message);
  ...
  async update(tableName, data) {
    try {
      ...
      console.error('[DataStoreService] Update error:', tableName, err.message);
