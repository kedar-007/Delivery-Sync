...
class NotificationService {
  ...
  async send({ toEmail, subject, htmlBody }) {
    ...
    console.log(`[NotificationService] send() → to=${toEmail} from=${fromEmail} subject="${subject}"`);
    ...
    try {
      ...
      console.log('[NotificationService] sendMail payload:', {
        from_email: payload.from_email,
        to_email: payload.to_email,
        subject: payload.subject,
        is_html: payload.is_html,
        content_length: htmlBody ? htmlBody.length : 0,
      });
      const result = await mail.sendMail(payload);
      console.log('[NotificationService] sendMail SUCCESS result:', JSON.stringify(result));
      return true;
    } catch (err) {
