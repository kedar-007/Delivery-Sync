...
class NotificationService {
  ...
  async sendInApp({ tenantId, userId, title, message, type, entityType = '', entityId = '', metadata = {} }) {
    ...
    try {
      ...
      // Fire-and-forget web push to active browser sessions
  ...
  async _sendWebPush(userId, title, message) {
    try {
      // sendNotification requires the Catalyst User Management ID (catalyst_user_id),
      // not the DataStore ROWID — look it up before sending.
