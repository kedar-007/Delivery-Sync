...
const VoiceAiInsights: React.FC<VoiceAiInsightsProps> = ({ summary, insights, onDismiss }) => {
  ...
  return (
    <div className="rounded-xl border border-violet-200 dark:border-violet-700/50 bg-gradient-to-br from-violet-50 to-blue-50 dark:from-violet-950/60 dark:to-slate-900/70 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-violet-100 dark:border-violet-800/40">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-violet-800 dark:text-violet-300">AI Insights</span>
          <span className="text-xs bg-violet-100 dark:bg-violet-800/40 text-violet-700 dark:text-violet-300 px-2 py-0.5 rounded-full font-medium">
            AI Generated
          </span>
        </div>
        <button
          type="button"
          onClick={onDismiss}
          className="p-1 rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-white/60 dark:hover:bg-white/10 transition-colors"
        >
          <X size={14} />
        </button>
      </div>
      ...
      <div className="px-4 py-3 space-y-3">
        {/* Summary */}
        {summary && (
          <MarkdownText text={summary} className="text-sm text-gray-700 dark:text-gray-200" accent="violet" />
        )}
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">{productivityScore}%</span>
            </div>
        ...
        {risks.length > 0 && (
          <div>
            ...
            <ul className="space-y-1">
              {risks.map((r, i) => (
                <li key={i} className="text-sm text-amber-800 dark:text-amber-300 flex items-start gap-2 bg-amber-50 dark:bg-amber-900/25 rounded-lg px-2.5 py-1.5">
        ...
        {suggestions.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-1.5 flex items-center gap-1">
              <Lightbulb size={11} /> Suggestions
            </p>
            <ul className="space-y-1">
              {suggestions.map((s, i) => (
                <li key={i} className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                  <Lightbulb size={13} className="text-blue-400 dark:text-blue-500 mt-0.5 shrink-0" />
                  {s}
                </li>
              ))}
            </ul>
