...
// Gradient colours keyed by first letter for consistent avatar colours
...
const ProfilePage = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Read as base64 and resize client-side to max 256px
    const reader = new FileReader();
    reader.onload = async (ev) => {
      const originalBase64 = ev.target?.result as string;

      // Resize to max 256×256 using a canvas
      const img = new Image();
      img.onload = async () => {
        const MAX = 256;
          ...
          <form onSubmit={handleSubmit(onSave)} className="space-y-4">
            ...
            <div>
              <label className="form-label flex items-center gap-1.5">
                <User size={13} className="text-gray-400" /> Display Name
              </label>
              <input
            ...
            <div>
              ...
              <input
                className="form-input bg-gray-50 cursor-not-allowed"
                value={(profile?.role ?? authUser?.role ?? '').replace(/_/g, ' ')}
                disabled
              />
