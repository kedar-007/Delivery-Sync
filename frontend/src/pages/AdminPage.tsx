...
const ROLE_PRESETS: Record<string, {
  ...
  techLead: {
    ...
    permissions: [
      ...
      'ATTENDANCE_READ','ATTENDANCE_WRITE','REGULARIZATION_APPROVE','LEAVE_READ','LEAVE_WRITE','LEAVE_TEAM_VIEW',
  ...
  hr: {
    ...
    permissions: [
      ...
      'ATTENDANCE_READ','ATTENDANCE_WRITE','ATTENDANCE_ADMIN','ATTENDANCE_TEAM_VIEW','ATTENDANCE_REPORT','REGULARIZATION_APPROVE','IP_CONFIG_WRITE',
  ...
  pm: {
    ...
    permissions: [
      ...
      'RAID_READ','RAID_WRITE','DECISION_READ','DECISION_WRITE',
      'STANDUP_READ','EOD_READ','TIME_READ','TIME_APPROVE',
      'ATTENDANCE_READ','ATTENDANCE_WRITE','REGULARIZATION_APPROVE','LEAVE_READ','LEAVE_WRITE','LEAVE_APPROVE','LEAVE_TEAM_VIEW',
  ...
  itAdmin: {
    ...
    permissions: [
      ...
      'ASSET_SCAN_FULL','ASSET_SCAN_BASIC',
...
const PERM_INFO: Record<string, { label: string; desc: string; risk: 'low' | 'medium' | 'high' }> = {
  ...
  TASK_COMMENT_DELETE:  { label: 'Delete Any Comment', desc: 'Delete any comment on any task (not just own)',                       risk: 'medium' },
  ...
  STANDUP_DELETE:     { label: 'Delete Any',         desc: 'Delete any standup entry (owner can always delete own)',                  risk: 'high' },
  ...
  EOD_DELETE:         { label: 'Delete Any',         desc: 'Delete any EOD report (owner can always delete own)',                     risk: 'high' },
  ...
  REGULARIZATION_APPROVE: { label: 'Approve Regularization', desc: 'Approve/reject attendance regularization requests of team peers (not just direct reports)', risk: 'medium' },
  ...
  TEAM_MANAGE:         { label: 'Create / Delete',    desc: 'Create new teams and delete existing teams',    risk: 'high' },
  BUG_REPORT_READ_ALL: { label: 'View All Reports',   desc: 'See bug reports across all projects/users',     risk: 'medium' },
  BUG_REPORT_CONFIG:   { label: 'Configure',          desc: 'Manage bug report settings and categories',     risk: 'high' },
  ...
  ASSET_SCAN_BASIC:   { label: 'Scan QR (Basic)',    desc: 'Scan asset stickers to see the owner',          risk: 'low'    },
  ASSET_SCAN_FULL:    { label: 'Scan QR (Full)',     desc: 'Scan to see full details, credentials, history', risk: 'high'  },
  ...
  AI_INSIGHTS:         { label: 'AI',          desc: 'Basic AI page: daily summary, suggestions, NLQ, voice',         risk: 'low' },
  AI_PERFORMANCE_SELF: { label: 'Self',        desc: 'Analyse OWN data only — no team picker, no other users',        risk: 'low' },
  AI_PERFORMANCE:      { label: 'My Team',     desc: 'Analyse own + own team(s) — NOT cross-team or org-wide',        risk: 'medium' },
